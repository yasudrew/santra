...
import AudioPreview from "@/components/songs/AudioPreview";
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row gap-6 mb-8">
        {/* Cover Image */}
        <div className="w-full sm:w-48 h-48 rounded-xl bg-gradient-to-br from-sky-500/20 to-teal-500/20 border border-border flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="flex flex-wrap gap-3">
            {song.song_key && (
              <span className="inline-flex items-center rounded-md bg-sky-500/10 border border-sky-500/20 px-2.5 py-1 text-xs font-medium text-sky-400">
                Key: {song.song_key}
              </span>
            )}
            {song.bpm && (
              <span className="inline-flex items-center rounded-md bg-teal-500/10 border border-teal-500/20 px-2.5 py-1 text-xs font-medium text-teal-400">
                {song.bpm} BPM
              </span>
            )}
            {song.time_signature && (
      ...
      {song.preview_url && (
        <div className="mb-8">
          <AudioPreview src={song.preview_url} title={song.title} />
        </div>
      )}
